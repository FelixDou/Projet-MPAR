...
class markov():
    ...
    def parcours(self,N, without_action = False, positionnel = False, file_name="visu_parcours"): # on parcours la chaine (en faisant N étapes)
        """ 
            ...
            for i in range(N):
                ...
                for j in range(len(poids)): # on recherche l'état cible
                    if choix <= poids[j]:
                        ...
                        break
                self.afficher(etat = etat_actif.state, file_name=f'image{i+1}')
    ...
    def simulation(self, n_iter, initial_state= "S0", mode_adv = "random" ):
